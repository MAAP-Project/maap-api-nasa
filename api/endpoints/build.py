...
def _validate_algorithm_name(algorithm_name):
    ...
    """
    log.debug(f"Validating algorithm name: {algorithm_name}")
    
    if not algorithm_name:
        log.debug("Validation failed: algorithm_name is empty or None")
        raise ValueError("algorithm_name is required")
    
    if len(algorithm_name) < 2 or len(algorithm_name) > 255:
        log.debug(f"Validation failed: algorithm_name length {len(algorithm_name)} is outside allowed range (2-255)")
    ...
    if not re.match(r'^[a-z0-9_-]+$', algorithm_name):
        log.debug(f"Validation failed: algorithm_name contains invalid characters")
        raise ValueError("algorithm_name can only contain lowercase letters, digits, hyphens (-), and underscores (_)")
    
    log.debug(f"Algorithm name validation successful: {algorithm_name}")
...
def _validate_algorithm_version(algorithm_version):
    ...
    """
    log.debug(f"Validating algorithm version: {algorithm_version}")
    
    if not algorithm_version:
        log.debug("Validation failed: algorithm_version is empty or None")
        raise ValueError("algorithm_version is required")
    
    if len(algorithm_version) > 128:
        log.debug(f"Validation failed: algorithm_version length {len(algorithm_version)} exceeds 128 character limit")
    ...
    try:
        ...
        log.debug("Algorithm version ASCII validation passed")
    except UnicodeEncodeError:
        log.debug("Validation failed: algorithm_version contains non-ASCII characters")
    ...
    if not re.match(r'^[a-zA-Z0-9_][a-zA-Z0-9._-]{0,127}$', algorithm_version):
        log.debug(f"Validation failed: algorithm_version does not match required pattern")
    ...
    log.debug(f"Algorithm version validation successful: {algorithm_version}")
...
def _validate_build_payload(payload):
    log.debug(f"Validating build payload with keys: {list(payload.keys()) if payload else 'None'}")
    
    # Required variables
    if payload.get("code_repository") is None:
        log.debug("Validation failed: code_repository is missing from payload")
        raise ValueError("code_repository is required")
    
    log.debug(f"Code repository: {payload.get('code_repository')}")
    
    # Image name - derive from algorithm_name if not provided
    algorithm_name = payload.get("algorithm_name")
    log.debug(f"Algorithm name from payload: {algorithm_name}")
    ...
    log.debug(f"Algorithm version from payload: {algorithm_version}")
    # Validate algorithm version format
    _validate_algorithm_version(algorithm_version)
    
    log.debug("Build payload validation completed successfully")


def _trigger_build_pipeline(payload, namespace):
    """Triggers a GitLab pipeline for the build request."""
    log.debug(f"Triggering build pipeline for namespace: {namespace}")
    log.debug(f"GitLab URL: {settings.GITLAB_URL}")
    log.debug(f"Project ID: {settings.GITLAB_BUILD_APP_PACK_PROJECT_ID}")
    log.debug(f"Pipeline ref: {settings.GITLAB_BUILD_APP_PACK_PIPELINE_REF}")
    
    try:
        log.debug("Initializing GitLab client")
        gl = gitlab.Gitlab(settings.GITLAB_URL, private_token=settings.GITLAB_BUILD_APP_PACK_PIPELINE_TOKEN)
        
        log.debug("Retrieving GitLab project")
        project = gl.projects.get(settings.GITLAB_BUILD_APP_PACK_PROJECT_ID)
        log.debug(f"Project retrieved: {project.name}")
        
        # Extract and validate required variables from payload
        variables = []
        log.debug("Processing pipeline variables")
        
        # Required variables
        repository_url = payload.get("code_repository")
        log.debug(f"Repository URL: {repository_url}")
        variables.append({"key": "REPOSITORY_URL", "value": repository_url})
        
        # Build command
        build_cmd = payload.get("build_command")
        if build_cmd:
            log.debug(f"Build command provided: {build_cmd}")
            variables.append({"key": "BUILD_CMD", "value": build_cmd})
        else:
            log.debug("No build command provided")
        
        # Base image with fallback
        base_image = payload.get("base_image")
        if base_image:
            log.debug(f"Base image: {base_image}")
            variables.append({"key": "BASE_IMAGE_NAME", "value": base_image})
        else:
            log.debug("No base image specified, using pipeline default")
        
        algorithm_name = payload.get("algorithm_name")
        image_name = f"{namespace}/{algorithm_name}"
        log.debug(f"Generated image name: {image_name}")
        variables.append({"key": "IMAGE_NAME", "value": image_name})
        
        algorithm_version = payload.get("algorithm_version")
        log.debug(f"Algorithm version: {algorithm_version}")
        ...
        log.debug("Encoding algorithm configuration to base64")
        algo_config_json = json.dumps(payload)
        log.debug(f"Algorithm config JSON length: {len(algo_config_json)} characters")
        algo_config_b64 = base64.b64encode(algo_config_json.encode()).decode()
        log.debug(f"Base64 encoded config length: {len(algo_config_b64)} characters")
        ...
        log.info(f"Successfully triggered GitLab pipeline ID: {pipeline.id}")
        log.debug(f"Pipeline web URL: {pipeline.web_url}")
        log.debug(f"Pipeline status: {pipeline.status}")
        log.debug(f"Pipeline ref: {pipeline.ref}")
        
        return pipeline
        
    except gitlab.exceptions.GitlabAuthenticationError as e:
        log.error(f"GitLab authentication failed: {e}")
        raise RuntimeError("Failed to authenticate with build service.")
    except gitlab.exceptions.GitlabGetError as e:
        log.error(f"GitLab project retrieval failed: {e}")
        raise RuntimeError("Failed to access build project. The build service may be misconfigured.")
    except gitlab.exceptions.GitlabCreateError as e:
        log.error(f"GitLab pipeline creation failed: {e}")
        raise RuntimeError("Failed to create build pipeline. Please check your build configuration.")
    except Exception as e:
        log.error(f"Unexpected error in GitLab pipeline trigger: {type(e).__name__}: {e}")
        log.debug(f"Full exception traceback: {traceback.format_exc()}")
...
def _create_and_commit_build(build_id, pipeline, repository_url, branch_ref, user):
    ...
    log.debug(f"Creating build record with ID: {build_id}")
    log.debug(f"User ID: {user.id}, Username: {user.username}")
    log.debug(f"Repository URL: {repository_url}")
    log.debug(f"Branch ref: {branch_ref}")
    log.debug(f"Pipeline ID: {pipeline.id if pipeline else None}")
    log.debug(f"Pipeline URL: {pipeline.web_url if pipeline else None}")
    log.debug(f"Initial status: {INITIAL_BUILD_STATUS}")
    ...
    log.debug("Adding build to database session")
    db.session.add(build)
    
    try:
        log.debug("Committing build record to database")
        db.session.commit()
        log.info(f"Successfully created build record {build_id} for user {user.username}")
    except Exception as e:
        log.error(f"Failed to commit build record to database: {e}")
        db.session.rollback()
        raise
...
def _update_build_status(build, req_data=None, query_pipeline=False):
    ...
    log.debug(f"Updating build status for build_id: {build.build_id if build else 'None'}")
    log.debug(f"Query pipeline flag: {query_pipeline}")
    log.debug(f"Request data provided: {req_data is not None}")
    
    status_code = status.HTTP_200_OK

    if build is None:
        log.debug("Build not found - returning 404")
    ...
    log.debug(f"Current build status: {current_status}")
    log.debug(f"Current pipeline URL: {pipeline_url}")
    ...
    if build.status not in BUILD_FINISHED_STATUSES:
        log.debug(f"Build status {build.status} is not finished, checking for updates")
        
        if query_pipeline:
            log.debug(f"Querying GitLab pipeline {build.pipeline_id} directly")
            try:
                gl = gitlab.Gitlab(settings.GITLAB_URL, private_token=settings.GITLAB_BUILD_APP_PACK_PIPELINE_TOKEN)
                project = gl.projects.get(settings.GITLAB_BUILD_APP_PACK_PROJECT_ID)
                ...
                log.debug(f"Retrieved pipeline status from GitLab: {updated_status}")
            except Exception as e:
                log.error(f"Failed to query GitLab pipeline {build.pipeline_id}: {e}")
                log.debug(f"Pipeline query error traceback: {traceback.format_exc()}")
                updated_status = build.status  # Fallback to current status on error
                log.debug("Falling back to current build status due to GitLab query error")
        else:
            log.debug("Extracting status from webhook payload")
            try:
                updated_status = req_data["object_attributes"]["status"]
                log.debug(f"Extracted status from webhook: {updated_status}")
            except (TypeError, KeyError) as e:
                log.debug(f"Failed to extract status from payload: {e}")
                log.debug(f"Payload structure: {req_data}")
        ...
        log.debug(f"Status mapping: {updated_status} -> {mapped_status}")
        current_status = mapped_status

        if current_status in BUILD_FINISHED_STATUSES:
            log.debug(f"Build reached final status: {current_status}")
        ...
        log.debug(f"Build already in finished status: {build.status}")

    response_body.update({"status": current_status})
    if pipeline_url:
        log.debug(f"Adding pipeline link to response: {pipeline_url}")
    ...
    log.debug(f"Returning response with status code: {status_code}")
...
class BuildList(Resource):
    ...
    def get(self):
        ...
        """
        log.debug("GET /build - Retrieving builds for current user")
        user = get_authorized_user()
        log.debug(f"Authorized user: {user.id} ({user.username})")
        
        # Query builds for the current user
        log.debug(f"Querying builds for user ID: {user.id}")
        builds = db.session.query(Build).filter_by(requester=user.id).order_by(Build.created.desc()).all()
        
        log.debug(f"Found {len(builds)} builds for user")
        
        build_list = []
        for i, build in enumerate(builds):
            log.debug(f"Processing build {i+1}/{len(builds)}: {build.build_id}")
            build_info = {
                "build_id": build.build_id,
                "status": build.status,
            ...
            if build.pipeline_url:
                log.debug(f"Adding pipeline link for build {build.build_id}")
                ...
                log.debug(f"No pipeline URL for build {build.build_id}")
            
            build_list.append(build_info)
        
        log.debug(f"Returning {len(build_list)} builds")
    ...
    def post(self):
        ...
        """
        log.debug("POST /build - Creating new build request")
        req_data = request.get_json()
        log.debug(f"Request payload keys: {list(req_data.keys()) if req_data else 'None'}")
        
        try:
            log.debug("Validating build payload")
            _validate_build_payload(req_data)
            
            user = get_authorized_user()
            log.debug(f"Authorized user for build: {user.id} ({user.username})")
            
            build_id = str(uuid.uuid4())
            log.debug(f"Generated build ID: {build_id}")
            
            # Trigger GitLab pipeline (validation happens inside)
            log.debug("Triggering GitLab pipeline")
        ...
        if pipeline:
            log.debug(f"Adding pipeline link to response: {pipeline.web_url}")
        ...
        log.info(f"Build request completed successfully: {build_id}")
...
class BuildStatus(Resource):
    ...
    def get(self, build_id):
        ...
        """
        log.debug(f"GET /build/{build_id} - Querying build status")
        build = db.session.query(Build).filter_by(build_id=build_id).first()
        
        if not build:
            log.debug(f"Build not found: {build_id}")
            return _generate_error("No build with that build ID found", status.HTTP_404_NOT_FOUND)
        
        log.debug(f"Found build {build_id}, checking permissions")
        
        # Check if user has permission to view this build
        user = get_authorized_user()
        log.debug(f"User {user.id} requesting build {build_id} (owner: {build.requester})")
        
        if build.requester != user.id:
            log.debug("User is not the build owner, checking admin privileges")
            # Check user is not admin
            if not hasattr(user, 'role_id'):
                log.debug("User has no role_id, access denied")
                return _generate_error("Access denied", status.HTTP_403_FORBIDDEN)
            if user.role_id != Role.ROLE_NAME_ADMIN:
                log.debug(f"User role {user.role_id} is not admin, access denied")
                return _generate_error("Access denied", status.HTTP_403_FORBIDDEN)
            log.debug("User has admin privileges, allowing access")
        else:
            log.debug("User is the build owner, allowing access")
        
        log.debug("Updating build status with pipeline query")
        response_body, status_code = _update_build_status(build, req_data=None, query_pipeline=True)
        log.debug(f"Returning build status with code: {status_code}")
        return response_body, status_code
    ...
    def post(self):
        ...
        try:
            ...
            if "object_attributes" in req_data and "status" in req_data["object_attributes"]:
                ...
                log.debug(f"Pipeline status from webhook: {webhook_status}")
                
        except (KeyError, TypeError) as e:
            log.debug(f"Failed to parse webhook payload: {e}")
            log.debug(f"Webhook payload structure: {req_data if 'req_data' in locals() else 'Failed to parse JSON'}")
        ...
        if build:
            log.debug(f"Found build {build.build_id} for pipeline {pipeline_id}")
        else:
            log.debug(f"No build found for pipeline {pipeline_id} in venue {settings.DEPLOY_PROCESS_EXECUTION_VENUE}")
        
        log.debug("Processing webhook status update")
        response_body, status_code = _update_build_status(build, req_data, query_pipeline=False)
        log.debug(f"Webhook processing complete, returning status: {status_code}")
        return response_body, status_code